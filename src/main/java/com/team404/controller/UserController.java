...
public class UserController {
	...
	public String home(@RequestParam(value = "category", required = false) String category,
		...
		List<ProductListDto> popularList = productService.findTopByFavoriteCount(5, category, loginUserNo);
	...
	public String accountForm(HttpSession session, Model model) {
		...
		// 최신 정보로 다시 조회 (DB → User)
		User user = userService.getUserByNo(loginUser.getUserNo());
	...
	public String submitAccount(@RequestParam("bankName") String bankName,
		...
		userService.updateAccount(loginUser.getUserNo(), bankName, accountNumber, accountHolder);
		// 세션 캐시도 갱신
		loginUser.setUserBankName(bankName);
		loginUser.setUserAccountNumber(accountNumber);
		loginUser.setUserAccountHolder(accountHolder);
		session.setAttribute("loginUser", loginUser);
		return "redirect:/mypage/account?saved=1";
	}

	// 구매내역 전체보기 (페이지당 15개)
	@GetMapping("/mypage/bought")
	public String boughtListPage(@RequestParam(value = "page", defaultValue = "1") int page, HttpSession session,
			Model model) {
		User loginUser = (User) session.getAttribute("loginUser");
		if (loginUser == null) {
			return "redirect:/login";
		...
		List<ProductListDto> boughtList = total > 0 ? productService.findBoughtListByBuyerNo(userNo, startNum, pageSize)
	...
	public String myReviewsPage(@RequestParam(value = "page", defaultValue = "1") int page, HttpSession session,
			Model model) {
		User loginUser = (User) session.getAttribute("loginUser");
		if (loginUser == null) {
			return "redirect:/login";
	...
	public String userPage(@PathVariable("userNo") int userNo,
		...
		List<Integer> boughtProductNos = new java.util.ArrayList<>();
		boolean hasBought = false;
		boolean allReviewed = false;

		if (loginUser != null) {
			boughtProductNos = orderService.findProductNosByBuyerAndSeller(loginUser.getUserNo(), userNo);
			System.out.println("boughtProductNos: " + boughtProductNos);
			hasBought = !boughtProductNos.isEmpty();
			if (hasBought) {
				allReviewed = true;
				for (Integer pNo : boughtProductNos) {
					boolean result = reviewService.existsReview(pNo, loginUser.getUserNo());
					System.out.println("pNo: " + pNo + ", existsReview: " + result);
					if (!result) {
						allReviewed = false;
					}
				}
		...
		session.removeAttribute("prevReviewContent");
		model.addAttribute("prevContent", prevContent);
		model.addAttribute("hasBought", hasBought);
		model.addAttribute("allReviewed", allReviewed);
		model.addAttribute("user", user);
		model.addAttribute("myProducts", myProducts);
	...
	public String kakaoCallback(@RequestParam("code") String code, HttpSession session) {
		try {
			...
			UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(user.getUserId(),
					null, authorities);
