...
public class FavoriteRepositoryImpl implements FavoriteRepository {
	...
	private final RowMapper<Favorite> favoriteRowMapper = new RowMapper<Favorite>() {
		...
		public Favorite mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			fav.setBoardNo(rs.getInt("product_no"));
	...
	public void insertFavorite(int userNo, int boardNo) {
		String sql = "insert into favorite (user_no, product_no) values (?, ?)";
		jdbcTemplate.update(sql, userNo, boardNo);
	}

	// 찜 삭제
	public void delete(int userNo, int boardNo) {
		String sql = "DELETE FROM favorite WHERE user_no=? AND product_no=?";
		jdbcTemplate.update(sql, userNo, boardNo);
	}

	// 찜 확인
	public boolean exists(int userNo, int boardNo) {
		String sql = "SELECT COUNT(*) FROM favorite WHERE user_no=? AND product_no=?";
	...
	public int countByBoard(int boardNo) {
		String sql = "SELECT COUNT(*) FROM favorite WHERE product_no=?";
		return jdbcTemplate.queryForObject(sql, Integer.class, boardNo);
	}

	// 내가 찜한 게시글 목록
	public List<Integer> findBoardNosByUser(int userNo) {
		String sql = "SELECT product_no FROM favorite WHERE user_no=?";
		...
		return jdbcTemplate.query(sql, mapper, userNo);
	}

	// 단건 조회 (필요하면)
	public Favorite findOne(int userNo, int boardNo) {
		String sql = "SELECT * FROM favorite WHERE user_no=? AND product_no=?";
		try {
