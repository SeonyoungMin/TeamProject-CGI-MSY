...
.app-container { max-width: 1300px; margin: 30px auto; padding: 0 20px; }
...
.btn-primary { background: #121212; border-color: #121212; color: #fff; }
.btn-primary:hover { background: #000; }
.btn-line { background: #fff; border-color: #121212; color: #121212; }
...
.top-bar a { color: #888; margin-right: 14px; }
...
.user-nick { font-size: 13px; font-weight: bold; }

.nav-link { font-size: 13px; color: #333; }
...
.noti-btn:hover, .noti-btn.active { background: #e8e8e8; }
...
.noti-dropdown-header span { font-size: 15px; font-weight: bold; color: #111; }

.noti-read-all-btn {
	font-size: 12px; color: #888;
	background: none; border: none; cursor: pointer; padding: 0;
}
.noti-read-all-btn:hover { color: #333; text-decoration: underline; }

.noti-drop-list { list-style: none; margin: 0; padding: 0; max-height: 380px; overflow-y: auto; }
...
.noti-drop-item:hover { background: #f9f9f9; }
.noti-drop-item:last-child { border-bottom: none; }
.noti-drop-item.unread { background: #fffbf0; border-left: 3px solid #f39c12; }
.noti-drop-item.unread:hover { background: #fff8e6; }
...
.noti-drop-icon.favorite { background: #e74c3c; }
.noti-drop-icon.comment  { background: #3498db; }
.noti-drop-icon.review   { background: #9b59b6; }
.noti-drop-icon.sold     { background: #27ae60; }
.noti-drop-icon.bought   { background: #2980b9; }
.noti-drop-icon.notice   { background: #e67e22; }
.noti-drop-icon.default  { background: #aaa; }

.noti-drop-body { flex: 1; min-width: 0; }
.noti-drop-msg { font-size: 13px; color: #222; line-height: 1.45; word-break: break-word; }
.noti-drop-time { font-size: 11px; color: #bbb; margin-top: 3px; }
.noti-drop-empty { text-align: center; padding: 36px 20px; color: #bbb; font-size: 13px; }

.noti-dropdown-footer { padding: 10px 16px; border-top: 1px solid #f0f0f0; text-align: center; }
.noti-dropdown-footer a { font-size: 13px; color: #555; }
.noti-dropdown-footer a:hover { color: #111; text-decoration: underline; }
</style>
						...
						<div class="noti-wrapper">
							...
							<div class="noti-dropdown" id="notiDropdown">
								<div class="noti-dropdown-header">
									<span>알림</span>
									<button type="button" class="noti-read-all-btn" id="notiReadAllBtn">전체 읽음</button>
								</div>
...
function validateSearch() {
	...
	if (keyword === '') { alert('검색어를 입력하세요.'); return false; }
	return true;
}
</script>
...
(function () {
	...
	function fetchBadgeOnly() {
		$.getJSON(ctx + '/notification/unread-count', function (data) { updateBadge(data.count || 0); });
	}
	...
	readAllBtn.addEventListener('click', function () {
		$.post(ctx + '/notification/read-all', function () { fetchNotifications(); });
	});
	...
	window.notiClick = function (notiNo, linkUrl) {
		$.post(ctx + '/notification/read', { no: notiNo }, function () {
			dropdown.classList.remove('open');
			btn.classList.remove('active');
			if (linkUrl && linkUrl.trim() !== '') window.location.href = linkUrl;
		});
	};
