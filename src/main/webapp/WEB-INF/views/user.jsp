...
<body>
	...
	<img src="<c:url value='/team404_upload/${user.userImageName}'/>" style="width: 30%"><br>
	파일명: ${user.userImageName}	<br>
	
	유저 No: ${user.userNo}<br>
	유저 ID:	${user.userId}<br>
	유저 PW:	${user.userPw}<br>
	유저 이름:	${user.userName}<br>
	유저 닉넴:	${user.userNickName}<br>
	유저 나이:	${user.userAge}<br>
	유저 주소:	${user.userAddress}<br>
	유저 전번:	${user.userPhone}<br>
	유저 등급:	${user.userGrade}<br>
	유저 권한:	${user.userRole}<br>
	유저생성T:	${user.userCreatedTime}<br>
	유저구매C:	${user.userBuyCount}<br>
	유저판매C:	${user.userSellCount}<br><br>
