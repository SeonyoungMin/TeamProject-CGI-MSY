...
<meta charset="UTF-8">
<title>거래 관리</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/orderPending.css">
</head>
<body>
	...
	<div class="pending-container">
		...
		<h3 class="is-orderPending-1">계좌이체 거래 요청</h3>
		<c:choose>
			<c:when test="${empty transferRequests}">
				<div class="empty-state is-orderPending-2">새로운 거래 요청이 없습니다.</div>
			</c:when>
			<c:otherwise>
				<c:forEach var="o" items="${transferRequests}">
					<div class="pending-card">
						<div class="pending-info">
							<div>
								<span class="pending-badge is-orderPending-3">거래 요청</span>
								<span class="is-orderPending-4">#${o.orderNo}</span>
							</div>
							<div class="product-name">${o.productName}</div>
							<div class="meta-row">
								금액 <strong><fmt:formatNumber value="${o.price}" />원</strong>
							</div>
							<div class="meta-row is-orderPending-5">
								<fmt:formatDate value="${o.createdTime}" pattern="yyyy-MM-dd HH:mm" /> 요청
							</div>
						</div>
						<div class="action-group">
							<form action="${ctx}/order/transfer/${o.orderNo}/approve" method="post" class="is-orderPending-6">
								<button type="submit" class="confirm-btn"
									onclick="return confirm('거래 요청을 승인하시겠어요?\n구매자에게 입금 폼 접근 권한이 부여됩니다.');">
									승인</button>
							</form>
							<form action="${ctx}/order/transfer/cancel" method="post" class="is-orderPending-7">
		...
		<h3 class="is-orderPending-8">계좌이체 승인 완료 (구매자 결제 대기)</h3>
		<c:choose>
			<c:when test="${empty approvedTransfers}">
				<div class="empty-state is-orderPending-9">결제를 기다리는 승인 거래가 없습니다.</div>
			</c:when>
			<c:otherwise>
				<c:forEach var="o" items="${approvedTransfers}">
					<div class="pending-card">
						<div class="pending-info">
							<div>
								<span class="pending-badge is-orderPending-10">승인 완료</span>
								<span class="is-orderPending-11">#${o.orderNo}</span>
							</div>
							<div class="product-name">${o.productName}</div>
							<div class="meta-row">
								금액 <strong><fmt:formatNumber value="${o.price}" />원</strong>
							</div>
							<div class="meta-row is-orderPending-12">구매자의 입금을 기다리는 중입니다.</div>
							<div class="meta-row is-orderPending-13">
								<fmt:formatDate value="${o.createdTime}" pattern="yyyy-MM-dd HH:mm" /> 승인
							</div>
						</div>
						<div class="action-group">
							<form action="${ctx}/order/transfer/cancel" method="post" class="is-orderPending-14">
								<input type="hidden" name="orderNo" value="${o.orderNo}">
								<button type="submit" class="cancel-btn"
		...
		<h3 class="is-orderPending-15">계좌이체 입금 대기</h3>
		<c:choose>
			<c:when test="${empty pendingTransfers}">
				<div class="empty-state is-orderPending-16">입금 대기 중인 거래가 없습니다.</div>
			</c:when>
			<c:otherwise>
				<c:forEach var="o" items="${pendingTransfers}">
					<div class="pending-card">
						<div class="pending-info">
							<div>
								<span class="pending-badge">입금대기</span> <span class="is-orderPending-17">#${o.orderNo}</span>
							</div>
							<div class="product-name">${o.productName}</div>
							<div class="meta-row">
							...
							<div class="meta-row">받는 사람: ${o.receiverName} ·
								${o.receiverPhone}</div>
							<div class="meta-row is-orderPending-18">
						...
						<div class="action-group">
							<form action="${ctx}/order/${o.orderNo}/confirm-payment"
								method="post" class="is-orderPending-19">
								<button type="submit" class="confirm-btn"
									onclick="return confirm('정말 입금이 확인됐나요?\n확인하면 거래가 완료 처리됩니다.');">
									입금 확인</button>
							</form>
							<form action="${ctx}/order/transfer/cancel"
								method="post" class="is-orderPending-20">
								<input type="hidden" name="orderNo" value="${o.orderNo}">
								<button type="submit" class="cancel-btn"
		...
		<h3 class="is-orderPending-21">직거래 예약</h3>
		<c:choose>
			<c:when test="${empty reservedDirects}">
				<div class="empty-state is-orderPending-22">예약된 직거래가 없습니다.</div>
			</c:when>
			<c:otherwise>
				<c:forEach var="o" items="${reservedDirects}">
					<div class="pending-card">
						<div class="pending-info">
							<div>
								<span class="pending-badge is-orderPending-23">직거래 예약</span>
								<span class="is-orderPending-24">#${o.orderNo}</span>
							</div>
							<div class="product-name">${o.productName}</div>
							<div class="meta-row">
								금액 <strong><fmt:formatNumber value="${o.price}" />원</strong>
							...
							<div class="meta-row is-orderPending-25">
								<fmt:formatDate value="${o.createdTime}" pattern="yyyy-MM-dd HH:mm" /> 예약
							</div>
						</div>
						<div class="action-group">
							<form action="${ctx}/order/direct/${o.orderNo}/complete" method="post" class="is-orderPending-26">
								<button type="submit" class="confirm-btn"
									onclick="return confirm('거래를 완료 처리하시겠어요?\n구매자에게 완료 알림이 발송됩니다.');">
									거래 완료</button>
							</form>
							<form action="${ctx}/order/direct/${o.orderNo}/cancel" method="post" class="is-orderPending-27">
								<button type="submit" class="cancel-btn"
